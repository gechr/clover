...
type cmdRun struct {
	...
	Enable     []string     `            help:"Resolve only these providers, skipping all others"                                         clib:"terse='Enable providers',group='Options/Selection/2'"  predictor:"provider" placeholder:"<provider>"`
	Disable    []string     `            help:"Skip these providers, resolving all others"                                                clib:"terse='Disable providers',group='Options/Selection/2'" predictor:"provider" placeholder:"<provider>"`
	Tags       []string     `name:"tag"  help:"Only process directives matching these tags"                                               clib:"terse='Filter by tags',group='Options/Selection/3'"    predictor:"tag"      placeholder:"<tag>"      short:"t" aliases:"tags"`
...
func (c *cmdRun) Run(configs *config.Resolver, workers parallelism) error {
	...
	providers, err := providerFilter(c.Enable, c.Disable)
	if err != nil {
		return err
	}
