...
func (c *cmdLogin) Help() string {
	return `Authenticates Clover with a provider and stores the minted token in your system keychain.

Later runs then read it from the credential chain instead of falling back to rate-limited anonymous access.

GitHub and GitLab use an OAuth device flow, where you authorize a one-time code in the browser, while Gitea uses a browser-based loopback flow.

Pass ` + "`--host`" + ` to authenticate against a GitHub Enterprise Server, self-managed GitLab, or self-hosted Gitea instance.

Such an instance runs its own OAuth app, so ` + "`--host`" + ` requires a matching ` + "`--client-id`" + ` (the public hosts use Clover's embedded app).`
}
