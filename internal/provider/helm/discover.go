...
// discoverRegistry lists a chart's versions from an OCI registry's tags via
// the shared client.
func (p *Provider) discoverRegistry(ctx context.Context, ref reference) ([]model.Candidate, error) {
	return provider.DiscoverOCITags(ctx, p.client, ref.repo, ref.String(), ref.url())
}

// candidate builds a model.Candidate from a chart version, parsing it for
// comparison; see [model.NewVariantCandidate]. A version that is not
