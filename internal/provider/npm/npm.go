...
// Identify returns the package name and its npmjs.com page. A custom registry
// has no public page, so its id links nowhere - matching URL - rather than to a
// same-named, unrelated public package.
func (p *Provider) Identify(r provider.Resource) (string, string) {
	...
	return res.pkg, constant.SchemeHTTPS + host + "/package/" + res.pkg
}
