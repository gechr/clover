...
- A tool version in a [mise](https://mise.jdx.dev) configuration file (`.mise.toml` or `mise.toml`). A HashiCorp product like `terraform = "1.9.8"` resolves with the [HashiCorp](hashicorp.md) provider and its inferred `product`, `node` resolves with the [Node.js](node.md) provider, `go` with the [Go](go.md) provider, `python` with the [Python](python.md) provider, and a `github:` or `ubi:` backend key (or one of the several hundred registry tools released on GitHub, like `tofu` or `ripgrep`) resolves with the [GitHub](github.md) provider and its inferred `repository`. A tool released on Codeberg, like `zig`, resolves with the [Gitea](gitea.md) provider. The tool-to-repository map is generated from the [mise registry](https://mise.jdx.dev/registry.html).
