...
- A `version` constraint inside a `required_providers` entry, resolved by the [Terraform](terraform.md) provider with the `source` read from the entry's own block - the one inference that looks beyond the target line, parsing the file as HCL to find the sibling `source` attribute. A `version` outside `required_providers` (a module block's, say) infers nothing. OpenTofu repositories should set `provider=opentofu` explicitly to resolve against the OpenTofu registry.
