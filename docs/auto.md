...
- A `target-version` in a `pyproject.toml` file, the compact `py314` form ruff, black, and mypy use, resolved by the [Python](python.md) provider. A `requires-python` floor is not inferred, since it declares a minimum supported version rather than the interpreter to track.
