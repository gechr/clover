...
Because `{version}` carries the full resolved value, the same follower works for a `build=` flavor - the `+ent.hsm.fips1403` suffix flows into both the `SHA256SUMS` URL and the artifact name. The checksum is refreshed only when the version it follows actually changes, so the two never drift out of step.
