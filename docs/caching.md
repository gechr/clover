...
- **Negative caching** - a stable client error (a 404 for a missing release, say) is remembered too, so repeated probes don't re-ask a question whose answer can't change mid-run. Retryable statuses (408, 429, 5xx) are never cached - a later attempt could clear them.
- **Error backoff** - a failed fetch (an unreachable host) is replayed to further callers for a short window, so a dead host costs one connection timeout, not one per marker.
...
- **Revalidation** - a stale entry carrying a validator (`ETag` or `Last-Modified`) is revalidated with a conditional request. When the origin answers `304 Not Modified` - the common case for registry and API responses - Clover reuses the stored body, paying for a header exchange instead of a full download. GitHub GraphQL responses, which carry no validators, get a short fallback freshness window (one minute) instead.
...
The disk tier is best-effort: if the cache directory can't be opened, Clover warns and runs with the in-memory tier alone - a broken cache never breaks a run.
...
2. The `CLOVER_NO_CACHE` environment variable - any non-empty value disables the cache.
3. The [`run.cache`](configuration.md) config key - set `false` to fetch everything fresh each run.
